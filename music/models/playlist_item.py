...
class PlaylistItemStates(StateGraph):
    ...
    @classmethod
    def handle_new(cls, instance: "PlaylistItem"):
        """
        Creates all needed fan-out objects for a new PlaylistItem.
        """
        ...
        if instance.type == PlaylistItem.Types.boost and instance.identity.local:
            FanOut.objects.create(
    ...
    @classmethod
    def handle_undone(cls, instance: "PlaylistItem"):
        """
        Creates all needed fan-out objects to undo a PlaylistItem.
        """
...
class PlaylistItem(StatorModel):
    """
    ...
    state = StateField(PlaylistItemStates)
    ...
    # Playlist item related properties
    number = models.IntegerField(default=0)
    name = models.TextField(null=True, blank=True)
    version = models.TextField(null=True, blank=True)
    media_type = models.CharField(default='music')
    creator_name = models.TextField(null=True, blank=True)
    release_name = models.TextField(null=True, blank=True)
    isrc = models.CharField(max_length=55, null=True, blank=True)
    ...
    def get_playlist_interactions(cls, playlists, identity):
        ...
        ids_with_interaction_type = cls.objects.filter(
            ...
            state__in=[PlaylistItemStates.new, PlaylistItemStates.fanned_out],
    ...
    @classmethod
    def create_votes(cls, playlist, identity, choices) -> list["PlaylistItem"]:
    ...
    @classmethod
    def by_ap(cls, data, create=False) -> "PlaylistItem":
        """
        Retrieves a PlaylistItem instance by its ActivityPub JSON object.
    ...
    def handle_undo_ap(cls, data):
        ...
        with transaction.atomic():
            ...
            interaction.transition_perform(PlaylistItemStates.undone_fanned_out)
    ...
    def handle_add_ap(cls, data):
        ...
        with transaction.atomic():
            ...
            return PlaylistItem.objects.get_or_create(
                type=cls.Types.pin,
                identity=identity,
                playlist=playlist,
                state__in=PlaylistItemStates.group_active(),
            )[0]
            ...
            try:
                ...
                for interaction in cls.objects.filter(
                    ...
                    interaction.transition_perform(
                        PlaylistItemStates.undone_fanned_out
                    )
            except (cls.DoesNotExist, Playlist.DoesNotExist):
                return
